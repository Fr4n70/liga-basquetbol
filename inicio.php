            ...
            <div class="col-lg-4 mb-4">
              <div class="card card-primary card-outline">
                <div class="card-header d-flex justify-content-between align-items-center">
                  ...
                  <button type="button" id="btnExportarPDF" class="btn btn-sm btn-danger">
                    <i class="bi bi-file-earmark-pdf-fill"></i> Exportar PDF
                  </button>
                </div>
                <div class="card-body p-0">
                  <table id="tablaPosiciones" class="table table-sm table-hover mb-0">
  ...
  <!-- Librerías para exportar la tabla de posiciones a PDF -->
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      botonExportar.addEventListener('click', function () {
        try {
          ...
          documento.text('Tabla de Posiciones', 14, 18);

          documento.setFontSize(10);
          documento.text('Generado el: ' + new Date().toLocaleString('es-PE'), 14, 25);

          documento.autoTable({
            html: '#tablaPosiciones',
            startY: 31,
            theme: 'grid',
            headStyles: {
              halign: 'center'
            ...
            didDrawPage: function (data) {
              const numeroPagina = documento.internal.getNumberOfPages();
              documento.setFontSize(9);
              documento.text(
                'Página ' + numeroPagina,
                documento.internal.pageSize.getWidth() - 28,
                documento.internal.pageSize.getHeight() - 10
              );
            }
          });

          documento.save('tabla_posiciones.pdf');
        } catch (error) {
          console.error(error);
          alert('No se pudo generar el PDF. Verifica tu conexión a internet e inténtalo nuevamente.');
        }
      });
    });
  </script>
